...
describe("all test for coursesSlice", () => {
  ...
  it("render the fetches correctly the courses data", async () => {
    ...
    expect(stateStore.courses[0]).toEqual(mockCourses[0]);
    expect(stateStore.courses[1]).toEqual(mockCourses[1]);
    expect(stateStore.courses[2]).toEqual(mockCourses[2]);
    expect(stateStore.courses[3]).toEqual(mockCourses[3]);
  });
